...
class UserController extends Controller
{
    ...
    {
        $response['users'] = User::with('province')->get();
    ...
    public function show($id)
    {
        ...
        if (Auth::user()->level != 'Administrador' && Auth::user()->id != $id) {
            ...
            $response['user'] = User::with('province')->find($id);
            //Logger
    ...
    public function update(Request $request, $id)
    {
        if (Auth::user()->level != 'Administrador' && Auth::user()->id != $id) {
            ...
            $request->validate([
                ...
                'password' => ['nullable', 'confirmed', Rules\Password::min(11)->mixedCase()->symbols()->numbers()],
            ]);
            User::find($id)->update([
                'name' => $request->name,
                'email' => $request->email,
                'level' => $request->level,
                'province_ref' => $request->province_ref,
